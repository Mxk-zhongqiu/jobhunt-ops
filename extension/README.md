...
## v0.3 · 常驻悬浮面板（气泡 ↔ 可拖动抽屉）

- **开启**：在 BOSS/猎聘 页面点扩展图标 → 顶栏「常驻本页」→ 面板接管并自动展开；弹窗自动关闭。
- **形态**：收起是**气泡**（可拖动）；点气泡展开为**抽屉面板**（内嵌与弹窗完全相同的功能界面：登录/新增/状态/AI 写手/探测），拖动标题栏可**自由移动**；「收起」回到气泡、「退出常驻」移除并同步同站其它标签页。
- **记忆**：常驻开关与位置均按站点（host）记忆——整页刷新后恢复为气泡；Boss 站内 SPA 切换页面组件不消失。
- 说明：点击页面其它区域**不会**自动收起（面板保持打开，由你手动收起/退出）。
