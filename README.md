...
| 投递追踪 | `/applications` | 100+ 家公司的分层 / 来源平台 / 状态机 / 截止日 / 投递链接（**核心页面**） |
| 投递统计 | `/stats` | 全量投递统计与分析：漏斗、分层/来源平台分布、转化率、阶段耗时、周投递节奏 vs 目标、最近状态动态 |
| 简历 | `/resume` | 素材库 + 多岗位版本（量化/视觉等）标签页切换、网页直接编辑、版本定制、导出 PDF/Markdown/HTML/纯文本、AI 经历改写 |
...
- `Application`：公司、分层（冲刺/主攻/保底）、来源平台（Boss直聘/猎聘/官网/牛客/应届生/学校就业网/内推/实习转正/其他平台）、岗位、状态机（计划投递→已投递→笔试→一面→二面→终面→Offer/已拒绝/放弃）、截止日、链接、JD 摘要（jdSummary）、状态时间线（statusHistory，状态变更自动记录）；
...
- `ResumeMaterial` + `ResumeVersion`（简历板块，独立存储）：素材库（单一数据源）+ 多岗位版本（挑选/排序/版本内微调），见 `src/types/resume.ts`。

**持久化**：主数据实时写入浏览器 localStorage；未登录用游客键 `jobhunt-ops-state-v1`，登录后按账号分区（`jobhunt-ops-state-v1:<uid>`，账号之间互不串扰），简历数据同理（`jobhunt-ops-resume-v1` / `...:<uid>`）；登录后均同步 Firestore（`states/{uid}` / `resumes/{uid}`，按 uid 隔离）。登出即切回游客槽，账号数据不会残留在游客界面；登录时检测到本机有未绑定的游客数据会先询问"并入账号 / 保留在游客区"，绝不静默上传。刷新不丢失。种子数据来自《求职规划内部文档》与 `resume-2026.md`，可直接修改。
...
- [x] 状态变化时间线（statusHistory 自动记录，/stats 阶段耗时统计）
- [x] 高频面试题文档沉淀（从 InterviewLog 自动汇总）
- [x] 简历版本管理（素材库 + 多岗位版本 + 导出 + AI 改写）
